...
class LineItems extends Component {
  ...
  onDelete(id) {
    ...
    this.props.updateMain({
      deleted: this.props.deleted.concat([id])
    })
  }

  render() {
    let Items = this.props.cartItems.filter(item => this.props.deleted.indexOf(item.id) === -1).map(item => {
      let ItemName = '';
      let Price = 0;
      ...
      return (
        <Item key={item.id} id={item.id} onDelete={id => this.onDelete(id)}>
          <View style={{
            ...
            <Text style={{
              flex: 6
            }}>{"$" + parseFloat(item.data.price * item.data.count).toFixed(2)}</Text>
          </View>
        </Item>
      );
    });
...
mapStateToProps = (state) => {
  return {totalCost: state.mainPage.totalCost, deleted: state.mainPage.deleted, cartItems: state.mainPage.cartItems, pizzaSizeNameArray: state.mainPage.pizzaSizeNameArray}
}
