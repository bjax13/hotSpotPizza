...
class Sides extends Component {
  componentDidMount() {
    axios.get('http://10.100.0.98:8888/api/sides/').then((response) => {
      ...
      for (var i = 2; i <= pages; i++) {
        axios.get(pageURL + i).then((response) => {
          ...
          this.props.updateMain({
            pizzaSidesArray: this.props.pizzaSidesArray.concat(response.data.results)
          })
          let newObj = {}
    ...
    let pizzaSides = this.props.pizzaSidesArray.map((sidesObj, i) => {
      ...
      decrement = () => {
        ...
        if (arr[i].count > 0) {
          ...
          if (this.props.customSidesArr.indexOf(arr[i].id) != -1) {
            this.props.updateMain({
              sidesCost: (this.props.sidesCost - (parseFloat(arr[i].price)))
            });
            this.props.updateMain({
              totalSidesCost: this.props.totalSidesCost - parseFloat(arr[i].price)
            });
          }
      ...
      increment = () => {
        ...
        if (this.props.customSidesArr.indexOf(arr[i].id) != -1) {
          ...
          this.props.updateMain({
            sidesCost: (this.props.sidesCost + (sidesObj.count * parseFloat(sidesObj.price)))
          });
          this.props.updateMain({
            totalSidesCost: this.props.totalSidesCost + parseFloat(sidesObj.price)
          });
        }
        this.forceUpdate()
      }
...
mapStateToProps = (state) => {
  return {sidesCost: state.mainPage.sidesCost, customSidesArr: state.mainPage.customSidesArr, totalSidesCost: state.mainPage.totalSidesCost, pizzaSidesArray: state.mainPage.pizzaSidesArray, orderHistorySidesObject: state.mainPage.orderHistorySidesObject}
}
