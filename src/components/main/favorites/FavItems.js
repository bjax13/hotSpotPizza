...
class FavItems extends Component {
  componentDidMount() {
    axios.get('http://10.100.0.98:8888/api/pizzas/').then((response) => {
      ...
      for (var i = 2; i <= pages; i++) {
        axios.get(pageURL + i).then((response) => {
          ...
          this.props.updateMain({
            pizzaArray: this.props.pizzaArray.concat(response.data.results)
          })
          let newObj = {}
  ...
  render() {
    ...
    let favPizzas = this.props.pizzaArray.filter((order) => order.public_display === true).map((pizzaObj, i) => {
      ...
      decrement = () => {
        ...
        if (arr[index].count > 0) {
          ...
          if (this.props.addToOrderArr.indexOf(arr[index].id) != -1) {
            this.props.updateMain({
              pizzaCost: (this.props.pizzaCost - (parseFloat(arr[index].price)))
            });
            this.props.updateMain({
              totalPizzaCost: this.props.totalPizzaCost - parseFloat(arr[index].price)
            });
          }
      ...
      increment = () => {
        ...
        if (this.props.addToOrderArr.indexOf(arr[index].id) != -1) {
          ...
          this.props.updateMain({
            pizzaCost: (this.props.pizzaCost + (pizzaObj.count * parseFloat(pizzaObj.price)))
          });
          this.props.updateMain({
            totalPizzaCost: this.props.totalPizzaCost + parseFloat(pizzaObj.price)
          });
        }
        this.forceUpdate()
      }
      ...
      return (
        <View key={pizzaObj.id} style={{
          ...
          <View style={{
            ...
            <TouchableHighlight onPress={decrement} style={{
              flex: 1
            }}>
              <Text>
                -
            ...
            <TouchableHighlight onPress={increment} style={{
              flex: 1
            }}>
              <Text>
                +
...
mapStateToProps = (state) => {
  return {pizzaCost: state.mainPage.pizzaCost, addToOrderArr: state.mainPage.addToOrderArr, totalPizzaCost: state.mainPage.totalPizzaCost, pizzaArray: state.mainPage.pizzaArray, orderHistoryPizzaObject: state.mainPage.orderHistoryPizzaObject}
}
